...
mito_path = "mtdnastat.250211.tsv"  # Update this with the correct file path
...
# Convert float-like integer columns to actual integers
int_columns = [
    "seq_date", "length", "length_emma", "seqlength_12s", "seqlength_16s", "seqlength_co1",
    "cds_no", "trna_no", "rrna_no", "rrna12s", "rrna16s", "atp6", "atp8", "cox1", "cox2", "cox3",
    "cytb", "nad1", "nad2", "nad3", "nad4", "nad4l", "nad5", "mad6", "trna_phe", "trna_val",
    "trna_leuuag", "trna_leuuaa", "trna_ile", "trna_met", "trna_thr", "trna_pro", "trna_lys",
    "trna_asp", "trna_glu", "trna_sergcu", "trna_seruga", "trna_tyr", "trna_cys", "trna_trp",
    "trna_ala", "trna_asn", "trna_gly", "trna_arg", "trna_his", "trna_gln"
]

for col in int_columns:
    mito[col] = pd.to_numeric(mito[col], errors='coerce').astype('Int64')  # Convert to nullable Int64

# Convert <NA> (Pandas Nullable Integers) to None
mito = mito.replace({pd.NA: None})
